...
//
//  Created by Keron Williams on 11/6/15.
...
class TipsTableViewController: UITableViewController, UITextFieldDelegate
{
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier("TipCell", forIndexPath: indexPath) as! TipTableViewCell
        ...
        {
            cell.textFeild.text = aTip["comment"] as? String
        }
    ...
    func textFieldShouldReturn(textField: UITextField) -> Bool
    {
        ...
        if textField.text != ""
        {
         rc = true
            ...
            let cell = contentView?.superview as? TipTableViewCell
            let indexPath = tableView.indexPathForCell(cell!)
            let aTip = tips[indexPath!.row]
            aTip["comment"] = textField.text
            aTip.saveInBackgroundWithBlock {(succeeded: Bool, error: NSError?) -> Void in
            if succeeded
                {
